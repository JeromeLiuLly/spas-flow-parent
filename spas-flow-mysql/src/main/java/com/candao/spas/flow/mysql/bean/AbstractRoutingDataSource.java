 ...
 * <p>
 * Abstract {@link DataSource} implementation that routes
...
abstract class AbstractRoutingDataSource extends AbstractDataSource implements InitializingBean {
	...
	/**
	 * Specify the map of target DataSources, with the lookup key as key. The mapped
	 * value can either be a corresponding {@link DataSource} instance or a data
	 ...
	 * <p>
	 * The mapped value can either be a corresponding {@link DataSource} instance or
	 * a data source name String (to be resolved via a {@link #setDataSourceLookup
	 * DataSourceLookup}).
	 * <p>
	 ...
	 * target DataSource map - simply falling back to the default DataSource in that
	 * case.
	 * <p>
	 * Switch this flag to "false" if you would prefer the fallback to only apply if
	 * the lookup key was {@code null}. Lookup keys without a DataSource entry will
	 * then lead to an IllegalStateException.
	 *
	 * @see #setTargetDataSources
	 * @see #setDefaultTargetDataSource
	 * @see #determineCurrentLookupKey()
	 ...
	 * {@link #setTargetDataSources targetDataSources} map, into the actual lookup
	 * key to be used for matching with the {@link #determineCurrentLookupKey()
	 * current lookup key}.
	 * <p>
	 ...
	 * <p>
	 * The default implementation handles DataSource instances and data source names
	 ...
	 * {@link #determineCurrentLookupKey() current lookup key}, performs a lookup in
	 * the {@link #setTargetDataSources targetDataSources} map, falls back to the
	...
	/**
	 * Determine the current lookup key. This will typically be implemented to check
	 * a thread-bound transaction context.
	 * <p>
	 * Allows for arbitrary keys. The returned key needs to match the stored lookup
	 * key type, as resolved by the {@link #resolveSpecifiedLookupKey} method.
	 */
	@Nullable
